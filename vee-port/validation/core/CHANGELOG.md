...
- Add ``ArrayCopyPerformance`` test measuring ``System.arraycopy`` throughput on large ``byte[]``
  buffers, with configurable minimum-throughput thresholds (aligned, misaligned, overlap-right, and
  overlap-left copies) and a configurable buffer size. See the README for configuration.
